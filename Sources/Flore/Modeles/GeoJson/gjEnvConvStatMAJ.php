<?php
    ...
    if (isset($_GET['bio_id'])) {
        $req = 'SELECT st_asgeojson(st_transform(sta_enveloppe_convexe, 4326)) FROM
            saisie.v_enveloppe_convexe_station_maj WHERE bio_id = ' . $_GET['bio_id'];
    }
    else {
        $req = 'SELECT st_asgeojson(st_transform(sta_enveloppe_convexe, 4326)) FROM
            saisie.v_enveloppe_convexe_station_maj';
    }
