//Fonction de traitement de l'affichage du type combinÃ© entier positif/boolÃ©en dans les grilles
...
//Fonction de rÃ©cupÃ©ration du nom de la photo d'aprÃ¨s celui du fichier
...
function renderIcon(val) {
    var result = '';
    if ((val) && !(nomPhoto(val) == '')) {
        result = Utf8.encode('<img src="' + CST_cheminRelatifPhoto + val + '" width=20\n\
            height=15 qtip="' + nomPhoto(val) + '">');
    }
    return result;
}
...
//Extension de classe pour masquer/afficher le contÃ´le de saisie dynamiquement
...
//Extension de classe pour changer le libellÃ© du contÃ´le de saisie dynamiquement
Ext.override(Ext.form.Field, {
...
//Fonction de traitement de l'affichage du type boolÃ©en dans les grilles
...
//Fonction de rÃ©cupÃ©ration d'un paramÃ¨tre GET
...
//Vtype contrÃ´lant que le texte se termine par une annÃ©e
...
//Extension de classe pour fixer la colonne de sÃ©lection Ã  gauche de la grille
...
function templateValidation(url, barreStatus, formulaire, fonctionRetour) {
    ...
    if (formulaire.form.isValid()) {
        ...
        formulaire.form.submit({
            url: url,
            method: 'POST',
            reset: false, // conservation de l'Ã©tat du formulaire en cas d'echec
        ...
        barreStatus.setStatus({
            clear: true, // faible attente pour Ãªtre Ã  nouveau "PrÃªt"
...
function createWorksheet(grid, types) {
    ...
    for (var i = 0; i < cm.getColumnCount(); i++) {
        if (!cm.isHidden(i) && cm.getColumnId(i) != 'checker') {
            ...
            if (fld.type == Ext.data.Types.AUTO) // forÃ§age des types passÃ©s en paramÃ¨tre pour les champs "AUTO""
            {
...
var Base64 = (function() {
    ...
    return {
        encode : (typeof btoa == 'function') ? function(input) {return btoa(Utf8.encode(input));} : function (input) {
            var output = "";
...
//Lecture de fichier en chaÃ®ne de caractÃ¨res
